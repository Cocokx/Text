...
public class Test : MonoBehaviour
{
    ...
    IEnumerator Locomotion_Ladder()
    {
        ...
        //播放动画  
        //anim.CrossFade(linkAnim, 0.1f, PlayMode.StopAll);
        blendTimeUp = Vector3.Distance(transform.position, linkEnd) ;
        ...
        do
        {
            ...
            transform.rotation = linkRotate;
            Debug.Log(Mathf.Abs(transform.position.y - linkEnd.y));
            yield return new WaitForSeconds(0);
            tblend += Time.deltaTime;
        } while (Mathf.Abs(transform.position.y-linkEnd.y)>minDis);
        //} while (anim[linkAnim].normalizedTime < 1);
    ...
    private void AnimationSetup()
    {
        ...
        anim["Jump"].speed = 2f;
        //anim["ClimbUp"].wrapMode = WrapMode.ClampForever;
        anim["ClimbUp"].speed = 2;
